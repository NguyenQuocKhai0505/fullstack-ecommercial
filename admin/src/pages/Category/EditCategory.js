...
const EditCategory = () => {
    ...
    //Load dữ liệu từ ID được truyền sang
    useEffect(() => {
        const loadCategoryData = async () => {
            try {
                setIsLoadingData(true);
                const response = await fetchDataFromApi(`/api/category/${id}`);
                if (response.success && response.data) {
                    ...
                    const existingImages = (category.images || []).map((img, index) => ({
                        id: `existing-${index}`,
                        url: img,
                        file: null
                    }));
        ...
        if(!formFields.name || !formFields.color ||uploadedImages.length===0 || !formFields.subCat) {
          ...
          try{
            ...
            if(response.success){
                ...
                context.showSnackbar("Failed to update category. Please try again.", "error");
            }
          }catch(error){
            setIsLoading(false);
            context.showSnackbar("Failed to update category. Please try again.", "error");
            ...
            <form className="form" onSubmit={updateCategory}>
                ...
                <div className="uploaded-images">
                    ...
                    <div 
                        ...
                        style={{ cursor: 'pointer', display: 'inline-block', padding: 8 }}
                    >
                ...
                <Button
                    ...
                    disabled={isLoading}
                >
                    {isLoading ? <CircularProgress color="inherit" size={24} /> : "UPDATE CATEGORY"}
                </Button>
            </form>
        </div>
    );
